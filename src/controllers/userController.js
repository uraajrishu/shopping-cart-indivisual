...
const login = async (req, res) => {
    ...
    } catch (error){res.status(500).send({status: !true, Message: error.message})}
}
//==============================================#put Api (update User) ==================================================>>
...
const update = async (req, res) => {
    try {// 💁‍♂️ get data from body
        ...
        if (!vfy.isEmptyVar(fname)) {user.fname = fname}

        if (!vfy.isEmptyVar(lname)) {user.lname = lname}
        ...
        if (!vfy.isEmptyVar(address)) {let addressObj = vfy.isValidJSONstr(address)
            ...
            if (!vfy.isEmptyObject(shipping)) {
                if (!vfy.isEmptyVar(shipping.street)) {user.address.shipping.street = shipping.street }

                if (!vfy.isEmptyVar(shipping.city)) {user.address.shipping.city = shipping.city}
            ...
            if (!vfy.isEmptyObject(billing)) {
                if (!vfy.isEmptyVar(billing.street)) {user.address.billing.street = billing.street}

                if (!vfy.isEmptyVar(billing.city)) {user.address.billing.city = billing.city}
