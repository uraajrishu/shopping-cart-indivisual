...
const create = async (req, res) => {
    try {// get body here
        ...
        // validate quantity
        if (isNaN(quantity)) return unsuccess(res, 400, '☹️ Quantity must be required!')
        if (typeof quantity != 'number') return unsuccess(res, 400, '☹️ Quantity must be a number!')
        if (Number(quantity) < 1) return unsuccess(res, 400, '☹️ Quantity value is > 1 !')
        ...
        if (!product) return unsuccess(res, 404, '☹️ productId not found!')
...
const update = async (req, res) => {
    try {// get body here
        ...
        // check if the cart is already exist or not
        const cart = await cartModel.findOne({ userId })
        if (!cart) return unsuccess(res, 404, '☹️ Cart not found!')
        // check both cartid's from req.body and db cart are match or not?
        if (cart._id != cartId) return unsuccess(res, 400, '☹️ CartId does\'t belong to this user!')
